...
//Asume somebody already initialized the display.
void LCD::internalBegin(uint8_t cols, uint8_t lines, uint8_t dotsize) 
{          
   if (lines > 1) 
   {
      _displayfunction |= LCD_2LINE;
   ...
   //backlight 
       setBacklight(HIGH,0);    //value,doWrite
      //_i2cio.write( _backlightStsMask );    //without write
...
void LCD::createChar(uint8_t location, const char *charmap)
{
   ...
   for (uint8_t i = 0; i < 8; i++)
   {
      write(*(charmap++)); //was  pgm_read_byte_near
      delayMicroseconds(40);
   }
}
...
void LCD::delayMicroseconds(int us){
 usleep(us);
     }
     
void LCD::delay(int s){
