...
//
// internalBegin
void LiquidCrystal_I2C::internalBegin(uint8_t cols, uint8_t lines, uint8_t dotsize) 
{      
   init();     // Initialise the I2C expander interface
   LCD::internalBegin ( cols, lines, dotsize );   
}
...
void LiquidCrystal_I2C::setBacklightPin ( uint8_t value, t_backlighPol pol = POSITIVE )
{
   ...
   _polarity = pol;
   setBacklight(BACKLIGHT_OFF,1);
}

//
// setBacklight
void LiquidCrystal_I2C::setBacklight( uint8_t value, int doWrite=1 ) 
{
...
int LiquidCrystal_I2C::init()
{
   ...
   if ( _i2cio.begin ( _Addr ) == 1 )
   {
      ...
      //_i2cio.write(HIGH);  //*** that, I like my ports HIGH
   }
   return ( status );
}
